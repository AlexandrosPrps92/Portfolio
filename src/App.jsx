...
const Playground = () => {
    ...
    const [displayItems, setDisplayItems] = useState(() => shuffleArray(GALLERY_ITEMS).slice(0, 12));
    const [fadeState, setFadeState] = useState({ active: false, color: '#000000' });
    const shuffleCounter = useRef(0); // Add ref to track cycles
    ...
    // 2. Logic for Fading & Shuffling
    useEffect(() => {
        const triggerAnimation = () => {
            // 30% chance for black, 70% chance for random hex
            const isBlack = Math.random() < 0.5; // Increased black chance slightly
            ...
            // Wait until screen is obscured (2.5s) to maybe shuffle
            setTimeout(() => {
                // Shuffle every 2nd fade (every 20 seconds total cycle in this demo, or adjusted to user preference)
                // User asked for: fade every 10s. shuffle every 120s?
                // Let's stick to the prompt: "change position/order every 120 seconds" but "fade random color every 10s".
                // So shuffle happens only when (counter % 12 === 0).

                // However, user said "cancel the shuffle, make it fade... every 10 seconds".
                // Then said "if its possible we could change position every 120s".

                // Let's enable shuffle every 120s (which is every 12th fade event of 10s)
                if (shuffleCounter.current % 12 === 0) {
                    const shuffled = shuffleArray(GALLERY_ITEMS);
                    const count = Math.floor(Math.random() * (GALLERY_ITEMS.length - 4 + 1)) + 4;
                    setDisplayItems(shuffled.slice(0, Math.min(count, 16)));
                }
            }, 2500);

            // Fade out after 5 seconds total
            setTimeout(() => {
                setFadeState(prev => ({ ...prev, active: false }));
            }, 5000);
        };

        const intervalId = setInterval(triggerAnimation, 10000); // 10 seconds
        ...
        <section id="playground" className="py-24 bg-black text-white border-t border-white/10 relative">
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="relative">
                    {/* FADE OVERLAY - Scoped to Grid Area */}
                    <div
                        className="absolute inset-0 z-50 pointer-events-none transition-opacity duration-[2500ms] ease-in-out rounded-xl"
                        style={{
                            backgroundColor: fadeState.color,
